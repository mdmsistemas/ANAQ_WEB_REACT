...
const ExportCsvVirtModal = ({virtConfig, getViSelectedPoint})  =>{ 
    ...
    const buildArrayCsv = () =>{
        ...
        let pointsArr = virtConfig.VirtualizacaoConfig.filter(it => it.vi === getViSelectedPoint())
        //console.log('pointsArr=', pointsArr)
        if(pointsArr[0]) {
            pointsArr[0].parametros.map(param =>{       
                csvData += ';' + param.nome   
            })
            pointsArr[0].variaveis.map(vari =>{
                csvData += ';' + 'DEP'
            })
            csvContent += csvData + "\r\n";
        }  
        ...
        pointsArr.map(point =>{
            csvContent += point.nomeVirtual + ";" 
            point.parametros.map(param =>{
                csvContent += param.valor + ';'
            })
            point.variaveis.map(vari =>{
                ...
                <div className="modal-body">
                    <div className="row justify-content-center"> 
                        <div className="row pb-2">
                            <SelectViForm resConfig={virtConfig} selectedVi={selectedVi} setSelectedVi={setSelectedVi}/>                                   
                        </div>                                         
                    </div>                     
                </div>                
...
export default ExportCsvVirtModal